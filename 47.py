...
if test==1:
    ...
    log_dir =  './compare32/'+str(num)+'volemptyaddspadbatch'

#load_dir =  './compare32/'+str(num)+'volemptyaddspade'
load_dir =0
if(load_dir!=0):
    test=1
...
class SPADE(nn.Module):
    def __init__(self, norm_nc, label_nc):
        ...
        self.param_free_norm = nn.BatchNorm2d(norm_nc, affine=False) #norm_nc is input size of channel
...
class SPADEResnetBlock(nn.Module):
    def __init__(self, fin, fout,label_nc): #fin is input channel, fout is output channel
        ...
        self.norm_0 = SPADE(fin,label_nc) #Spade 1:input channel 2:seg channel
...
class Generator(nn.Module):
    ...
    def __init__(self,seg_label):
        ...
        # encode
        self.param_batch_norm0 = nn.BatchNorm2d(3, affine=False)
        self.c1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
        self.a1 = nn.ReLU() 
        self.c2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.param_batch_norm1 = nn.BatchNorm2d(64, affine=False)  # norm_nc is input size of channel
        self.a2 = nn.ReLU()
        ...
        self.param_batch_norm2 = nn.BatchNorm2d(128, affine=False)  # norm_nc is input size of channel
        self.a4 = nn.ReLU()
        ...
        self.param_batch_norm3 = nn.BatchNorm2d(256, affine=False)  # norm_nc is input size of channel
        self.a8 = nn.ReLU()
        ...
        self.b1 = nn.Conv2d(513, 512, kernel_size=3, stride=1, padding=1) #>512*4*4
        self.sp0 = SPADEResnetBlock(512, 256, seg_label)
        self.sp1 = SPADEResnetBlock(256, 128, seg_label)
        #8*8
        self.sp2 = SPADEResnetBlock(128, 64, seg_label)
        #16*16
        self.sp3 = SPADEResnetBlock(64, 2, seg_label)
        #32*32
        self.conv_img = nn.Conv2d(2, 1, kernel_size=3, stride=1, padding=1)
        
        self.up = nn.Upsample(scale_factor=2)
        
        self.bn = nn.Sigmoid() #勾配消失問題に関わるから不適切

        self.upforvol=nn.Upsample(scale_factor=8,mode='nearest')
    ...
    def forward(self, bf,vol,seg):
        ...
        x=torch.cat((bf,seg),dim=1)
        x=self.param_batch_norm0(x)
        x = self.c1(x)
        x = self.a1(x)
        x = self.c2(x)
        x=self.param_batch_norm1(x)
        x = self.a2(x)
        x = self.m1(x)
        x = self.c3(x)
        x = self.a3(x)
        x = self.c4(x)
        x=self.param_batch_norm2(x)
        x = self.a4(x)
        x=self.m2(x)
        x = self.c7(x)
        x = self.a7(x)
        x = self.c8(x)
        x=self.param_batch_norm3(x)
        x = self.a8(x)
        x = self.m4(x)
        x = self.c9(x)
        x = self.a9(x)
        ...
        vol=self.upforvol(vol)
        seg=torch.cat((bf,seg*vol),dim=1)
...
print('param:'+str(get_n_params(G)))
...
# for images
rBfx = pd.read_csv(filepath_or_buffer="/home/user/Desktop/forconfirm32(42000)/32bodyfx.csv",header=None)
rBfy=pd.read_csv(filepath_or_buffer="/home/user/Desktop/forconfirm32(42000)/32bodyfy.csv",header=None)
rEmpty=pd.read_csv(filepath_or_buffer="/home/user/Desktop/forconfirm32(42000)/32empty.csv",header=None)
rVol=pd.read_csv(filepath_or_buffer="/home/user/Desktop/forconfirm32(42000)/32vol.csv",header=None)
rRho=pd.read_csv(filepath_or_buffer="/home/user/Desktop/forconfirm32(42000)/32rho.csv",header=None)
...
rBf,rRho,rVol,rEmpty=torch.from_numpy(rBf),torch.from_numpy(rRho),torch.from_numpy(rVol),torch.from_numpy(rEmpty)
...
'''
for i in range(rEmpty.size()[0]):
    save_image(1-rEmpty[i,:,:,:], os.path.join(log_dir, 'Empty{}.png'.format(i)))
'''
if test==1:
    # test
    Bfx = pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32bodyfx.csv",header=None,nrows=32*20*batch_size)
    Bfy=pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32bodyfy.csv",header=None,nrows=32*20*batch_size)
    Empty=pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32empty.csv",header=None,nrows=32*20*batch_size)
    Vol=pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32vol.csv",header=None,nrows=20*batch_size)
    Rho=pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32rho.csv",header=None,nrows=32*20*batch_size)
else:
    # load input dataset
    Bfx = pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32bodyfx.csv",header=None)
    Bfy=pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32bodyfy.csv",header=None)
    Empty=pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32empty.csv",header=None)
    Vol=pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32vol.csv",header=None)
    Rho=pd.read_csv(filepath_or_buffer="/home/user/Desktop/alldata(add32)/32rho.csv",header=None)
...
train = torch.utils.data.TensorDataset(torch.from_numpy(Bf_train),torch.from_numpy(Rho_train),torch.from_numpy(Vol_train),torch.from_numpy(Empty_train))
input_loader = torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=True)
test = torch.utils.data.TensorDataset(torch.from_numpy(Bf_test),torch.from_numpy(Rho_test),torch.from_numpy(Vol_test),torch.from_numpy(Empty_test))
test_loader = torch.utils.data.DataLoader(test, batch_size=tbatch_size, shuffle=True)
...
if(load_dir==0):
    for epoch in range(num_epochs):
        ...
        if test_loss < best_loss:
            ...
            if torch.cuda.device_count() > 1:
                torch.save(G.module.state_dict(), os.path.join(log_dir, 'GoodG.pth'))
            else:
                torch.save(G.state_dict(), os.path.join(log_dir, 'GoodG.pth'))       
            '''for i in range(samples.size()[0]):
                A=samples[i,0,:,:]
                with open(log_dir+'/'+str(epoch)+'resultmatlab'+str(num)+'.csv', 'a') as f_handle:
                    np.savetxt(f_handle, A)'''
            


        print('epoch %d, G_loss: %.4f test_loss: %.4f' % (epoch + 1,G_loss,test_loss))
        ...
        if torch.cuda.device_count() > 1:
            torch.save(G.module.state_dict(), os.path.join(log_dir, 'lastG.pth'))
        else:
